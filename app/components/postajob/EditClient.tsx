...
const EditClient:React.FC<EditClientProps> = ({image, companyName, jobTitle,setJobTitle,selectedLocations,setSelectedLocations,jobType, setJobType, experienceLevel, setExperienceLevel, 
  educationalDegree, setEducationalDegree, min, setMin, max, setMax, visa, setVisa, description, setDescription, setPage }) => {
  ...
  return (
    <div className={styles.container}>
        ...
        <div className={`${styles.centerDiv} ${styles.SelectionGroup}`}>
          <div className={`${styles.DropDownContainerDiv} ${styles.DropDownContainerDivMargin}`}>
            <Dropdown id='JobType' value={jobType} list={jobTypes}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setJobType(e.target.value)}  />
          </div>
          <div className={`${styles.DropDownContainerDiv} ${styles.DropDownContainerDivMargin}`}>
            <Dropdown id='ExperienceLevel' value={experienceLevel} list={experienceLevels}
          ...
          <div className={`${styles.DropDownContainerDiv}`}>
            <Dropdown id='EducationalDegree' value={educationalDegree} list={educationalDegrees}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setEducationalDegree(e.target.value)}  />
          </div>
        </div>
        ...
        <div className={`${styles.SalaryGroup}`}>
          ...
          <div className={`${styles.MaxMinContainerDiv}`}>
            <Input id="Min" placeholder="Min" type='number' required
          ...
          <div className={`${styles.MaxMinContainerDiv}`}>
            <Input id="Max" placeholder="Max" type='number' required
        ...
        <div className={`${styles.Continue}`}>
          <Button onClick={Review} text="Save and Review"
          paddingTop={16} paddingBottom={16} paddingLeft={27}
          paddingRight={28} fontSize={15} />
        </div>
    </div>
  )
}
